...
class E_attr_concat(nn.Module):
  def __init__(self, input_dim_a, input_dim_b, output_nc=8, norm_layer=None, nl_layer=None):
    ...
    conv_layers_A = conv_layers_A + [nn.Conv2d(input_dim_a, ndf, kernel_size=4, stride=2, padding=0, bias=True)]
    for n in range(1, n_blocks):
      input_ndf = ndf * min(max_ndf, n)  # 2**(n-1)
      output_ndf = ndf * min(max_ndf, n+1)  # 2**n
      conv_layers_A = conv_layers_A + [BasicBlock(input_ndf, output_ndf, norm_layer, nl_layer)]
    conv_layers_A = conv_layers_A + [nl_layer(), nn.AdaptiveAvgPool2d(1)] # AvgPool2d(13)
